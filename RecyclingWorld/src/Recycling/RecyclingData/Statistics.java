...
//            System.out.println(message);
    ...
    public static double timesUsed(String id) throws SQLException, ClassNotFoundException {
        String sql = "SELECT COUNT(*) AS TOTAL FROM TRANSACTIONS WHERE ID='"+id+"' AND EMPTIED='0'";
        double total = doubleQuery(sql,"Finding total number of empties performed on Machine ID "+id);
        return total;
    }
    ...
    public static String leastUsedByWeight() throws SQLException, ClassNotFoundException {
        String sql = "SELECT DISTINCT SUM(item_weight) AS TOTAL,ID AS ANSWER FROM TRANSACTIONS WHERE ID>'0' GROUP BY ANSWER ORDER BY TOTAL ASC LIMIT 1";
        String answer = stringQuery(sql,"Finding machine with most recycled by weight.");
        return answer;
    }
    ...
    public static String leastUsedByPayout() throws SQLException, ClassNotFoundException {
        String sql = "SELECT DISTINCT SUM(money) AS TOTAL,ID AS ANSWER FROM TRANSACTIONS WHERE ID>'0' GROUP BY ANSWER ORDER BY TOTAL ASC LIMIT 1";
        String answer = stringQuery(sql,"Finding machine with most recycled by weight.");
        return answer;
    }
